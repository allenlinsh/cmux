...
extension Array where Element == CuratedSettingEntry {
    ...
    public static func cmuxDefault(catalog: SettingCatalog) -> [CuratedSettingEntry] {
        [
            ...
            .init(section: .browser, id: "dev-host-no-cache", title: String(localized: "settings.browser.devHostNoCache", defaultValue: "Always Fetch Fresh Content from Local Dev Servers"), synonyms: "Always Fetch Fresh Content from Local Dev Servers browser.disableCacheForDevHosts localhost cache stale fresh hard reload dev server no-cache turbopack vite next"),
